...
public class ConfigurationValues {
    ...
    public boolean reload() {
        ...
        for (String key: config.getKeys(false)) {
            final Key match = Key.fromString(key);
            if (match == null) {
                plugin.getLogger().warning("The config option '" + key + " was found in the config, but is invalid.");
                continue;
            }
            final var value = config.getObject(key, match.valueType);
            if (value == null || value.getClass() != match.valueType) {
				final String classDisplay = (value == null) ? "null" : value.getClass().toString();
